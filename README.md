...
| Device search (`searchDevices`) | Implemented — enrolled devices via `managedDevices` (exact-match `eq` filter across serial number, device name, UPN, and email — `contains()`/advanced query support proved unreliable on a real tenant, and `userDisplayName` isn't filterable at all, see code comments), falling back to Windows Autopilot / Apple ADE pre-enrollment identities |
