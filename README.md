...
| Device search (`searchDevices`) | Implemented — enrolled devices via `managedDevices` (exact-match `eq` filter across serial number, device name, UPN, email, and user display name — `contains()`/advanced query support proved unreliable on a real tenant, see code comment), falling back to Windows Autopilot / Apple ADE pre-enrollment identities |
